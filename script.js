...
const filterState = { category: "all", skin: "all", price: "all" };

function loadSavedReviews() {
  const savedReviews = localStorage.getItem(reviewStorageKey);
  if (!savedReviews) return defaultReviews.slice();
  try {
...
function saveReviewsToStorage() { localStorage.setItem(reviewStorageKey, JSON.stringify(reviews)); }
function formatRupiah(value) { return "Rp" + Number(value).toLocaleString("id-ID"); }
function formatLabel(value) {
  const labels = { moisturizer: "Moisturizer", sunscreen: "Sunscreen", serum: "Serum", toner: "Toner", lip: "Lip Care", acne: "Acne", normal: "Normal", dry: "Dry", brightening: "Brightening", "anti-aging": "Anti-aging", all: "Semua" };
  return labels[value] || value;
}
function safeText(value) {
  return String(value).replace(/[&<>"]/g, function (char) { return { "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[char]; });
}
function getProductSkins(product) { return Array.isArray(product.skins) ? product.skins : [product.skin || "all"]; }
...
function initCatalogPage() {
  ...
  renderFilterButtons("categoryFilters", "category", [{ label: "Semua", value: "all" }, { label: "Moisturizer", value: "moisturizer" }, { label: "Sunscreen", value: "sunscreen" }, { label: "Serum", value: "serum" }, { label: "Toner", value: "toner" }, { label: "Lip Care", value: "lip" }]);
  renderFilterButtons("skinFilters", "skin", [{ label: "Semua", value: "all" }, { label: "Acne", value: "acne" }, { label: "Normal", value: "normal" }, { label: "Dry", value: "dry" }, { label: "Brightening", value: "brightening" }, { label: "Anti-aging", value: "anti-aging" }]);
  renderFilterButtons("priceFilters", "price", [{ label: "Semua", value: "all" }, { label: "< 50K", value: "under-50000" }, { label: "50K - 100K", value: "50000-100000" }, { label: "100K - 200K", value: "100000-200000" }, { label: "> 200K", value: "over-200000" }]);
  ...
  function getFilteredProducts() {
    ...
    let filteredProducts = products.filter(function (product) {
      ...
      const searchMatch = product.name.toLowerCase().includes(keyword) || product.description.toLowerCase().includes(keyword) || formatLabel(product.category).toLowerCase().includes(keyword) || skinText.includes(keyword);
      const categoryMatch = filterState.category === "all" || product.category === filterState.category;
      const skinMatch = filterState.skin === "all" || productSkins.includes(filterState.skin);
      let priceMatch = true;
      if (filterState.price === "under-50000") priceMatch = product.price < 50000;
      if (filterState.price === "50000-100000") priceMatch = product.price >= 50000 && product.price <= 100000;
      if (filterState.price === "100000-200000") priceMatch = product.price > 100000 && product.price <= 200000;
      if (filterState.price === "over-200000") priceMatch = product.price > 200000;
      return searchMatch && categoryMatch && skinMatch && priceMatch;
    });
    if (sortSelect.value === "az") filteredProducts.sort(function (a, b) { return a.name.localeCompare(b.name); });
    if (sortSelect.value === "za") filteredProducts.sort(function (a, b) { return b.name.localeCompare(a.name); });
    if (sortSelect.value === "low") filteredProducts.sort(function (a, b) { return a.price - b.price; });
    if (sortSelect.value === "high") filteredProducts.sort(function (a, b) { return b.price - a.price; });
  ...
  function renderCatalog() {
    ...
    if (filteredProducts.length === 0) {
      productGrid.innerHTML = '<p class="col-span-full rounded-2xl border border-[#dfd2c7] bg-white/70 p-8 text-center text-taupe">Produk tidak ditemukan. Coba kata kunci atau filter lain.</p>';
      return;
    }
    productGrid.innerHTML = filteredProducts.map(function (product) {
      const productIndex = products.indexOf(product);
      const tags = getProductSkins(product).map(function (skin) { return '<span class="rounded-full bg-blush px-3 py-1 text-sm font-bold text-espresso">' + formatLabel(skin) + '</span>'; }).join("");
      return '<article class="overflow-hidden rounded-3xl border border-[#dfd2c7] bg-white/75 shadow-xl shadow-[#563a2b]/10">' +
        '<img src="' + safeText(product.image) + '" alt="' + safeText(product.name) + '" class="h-64 w-full object-cover">' +
        '<div class="p-5"><p class="text-xs font-bold uppercase tracking-[.18em] text-terracotta">' + formatLabel(product.category) + '</p><h3 class="mt-2 text-xl font-bold">' + safeText(product.name) + '</h3><p class="mt-3 leading-7 text-taupe">' + safeText(product.description) + '</p><p class="mt-4 text-lg font-bold text-espresso">' + formatRupiah(product.price) + '</p><p class="mt-1 font-bold text-terracotta">Rating ' + product.rating + '/5</p><div class="mt-4 flex flex-wrap gap-2">' + tags + '</div><a href="detail.html?id=' + productIndex + '" class="mt-5 inline-flex w-full justify-center rounded-full bg-terracotta px-5 py-3 font-bold text-white transition hover:bg-espresso">Klik Detail</a></div></article>';
    }).join("");
  }
...
function renderFilterButtons(targetId, group, options) {
  ...
  wrapper.innerHTML = options.map(function (option) {
    return '<button type="button" class="filter-btn rounded-full border border-[#dfd2c7] px-4 py-2 font-bold transition" data-filter-group="' + group + '" data-filter-value="' + option.value + '">' + option.label + '</button>';
  }).join("");
...
function updateActiveButtons() {
  document.querySelectorAll(".filter-btn").forEach(function (button) {
    const active = filterState[button.dataset.filterGroup] === button.dataset.filterValue;
    button.className = active ? "filter-btn rounded-full border border-terracotta bg-terracotta px-4 py-2 font-bold text-white transition" : "filter-btn rounded-full border border-[#dfd2c7] bg-white px-4 py-2 font-bold text-taupe transition hover:border-terracotta hover:text-terracotta";
  });
}
...
function initDetailPage() {
  ...
  if (!product) {
    detailContent.innerHTML = '<div class="rounded-3xl border border-[#dfd2c7] bg-white/75 p-8 text-center"><h2 class="font-serif text-4xl">Produk tidak ditemukan.</h2><a href="catalog.html" class="mt-6 inline-flex rounded-full bg-terracotta px-5 py-3 font-bold text-white">Kembali ke katalog</a></div>';
    return;
  }
  detailContent.innerHTML = '<div class="grid gap-8 lg:grid-cols-[.95fr_1.05fr] lg:items-start">' +
    '<div class="overflow-hidden rounded-3xl border border-[#dfd2c7] bg-white/75 shadow-xl shadow-[#563a2b]/10"><img src="' + safeText(product.image) + '" alt="' + safeText(product.name) + '" class="max-h-[620px] w-full object-cover"></div>' +
    '<div class="rounded-3xl border border-[#dfd2c7] bg-white/75 p-6 shadow-xl shadow-[#563a2b]/10"><p class="text-xs font-bold uppercase tracking-[.22em] text-terracotta">Detail Produk</p><h2 class="mt-3 font-serif text-5xl leading-tight">' + safeText(product.name) + '</h2><p class="mt-4 text-taupe leading-8">' + safeText(product.description) + '</p><div class="mt-6 grid gap-3 sm:grid-cols-2"><div class="rounded-2xl bg-cream p-4"><p class="text-sm font-bold text-taupe">Harga</p><p class="text-2xl font-bold text-espresso">' + formatRupiah(product.price) + '</p></div><div class="rounded-2xl bg-cream p-4"><p class="text-sm font-bold text-taupe">Rating</p><p class="text-2xl font-bold text-terracotta">' + product.rating + '/5</p></div></div><div class="mt-6 rounded-2xl bg-cream p-5"><h3 class="text-xl font-bold">Ingredients</h3><p class="mt-2 leading-7 text-taupe">' + safeText(product.ingredients) + '</p></div><div class="mt-4 rounded-2xl bg-cream p-5"><h3 class="text-xl font-bold">Manfaat</h3><p class="mt-2 leading-7 text-taupe">' + safeText(product.benefits) + '</p></div><a href="catalog.html" class="mt-6 inline-flex rounded-full bg-terracotta px-6 py-3 font-bold text-white transition hover:bg-espresso">Kembali ke katalog</a></div></div>';
}

function initReviewPage() {
  const reviewForm = document.getElementById("reviewForm");
  const reviewTableBody = document.getElementById("reviewTableBody");
  if (!reviewForm || !reviewTableBody) return;
  const customerName = document.getElementById("customerName");
  const customerSkin = document.getElementById("customerSkin");
  ...
  function renderReviews() {
    reviewTableBody.innerHTML = reviews.map(function (review, index) {
      return '<tr class="border-b border-[#dfd2c7]"><td class="p-4">' + (index + 1) + '</td><td class="p-4 font-bold">' + safeText(review.name) + '</td><td class="p-4">' + formatLabel(review.skin) + '</td><td class="p-4">' + formatLabel(review.category) + '</td><td class="p-4 font-bold text-terracotta">' + review.rating + '/5</td><td class="p-4 text-taupe">' + safeText(review.comment) + '</td></tr>';
    }).join("");
  }
  function clearReviewReminders() {
    customerSkin.setCustomValidity("");
    customerCategory.setCustomValidity("");
  }
  function checkReviewChoices() {
    customerSkin.setCustomValidity(customerSkin.value ? "" : "Kondisi kulit wajib dipilih terlebih dahulu.");
    customerCategory.setCustomValidity(customerCategory.value ? "" : "Produk yang dicari wajib dipilih terlebih dahulu.");
    return customerSkin.value && customerCategory.value;
  }
  reviewForm.addEventListener("submit", function (event) {
    event.preventDefault();
    if (!checkReviewChoices()) {
      reviewForm.reportValidity();
      return;
    }
    const reviewData = { name: customerName.value.trim(), skin: customerSkin.value, category: customerCategory.value, rating: customerRating.value, comment: customerComment.value.trim() };
    reviews.push(reviewData);
    saveReviewsToStorage();
    reviewForm.reset();
    clearReviewReminders();
    renderReviews();
    window.location.href = "catalog.html";
  });
  customerSkin.addEventListener("change", checkReviewChoices);
  customerCategory.addEventListener("change", checkReviewChoices);
  renderReviews();
}
